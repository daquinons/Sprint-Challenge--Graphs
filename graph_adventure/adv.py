...
def not_visited(curr_path, room, player):
    visited = set()
    queue = Queue()
    path = [room]
    queue.enqueue(path)
    ...
    while queue.size() > 0:
        ...
        if v not in visited:
            ...
            for neighbors in curr_path[v]:
                n = curr_path[v][neighbors]
                n_p = p.copy()
                n_p.insert(0, n)
                queue.enqueue(n_p)
    ...
    while len(path) > 1:
        ...
        for branch in curr_path[current]:
            if curr_path[current][branch] == path[-1]:
                player_move_dirs.append(branch)

    for direction in player_move_dirs:
        player.travel(direction)

    return player_move_dirs


visited = {}

while len(visited) < room_count:
    room = player.currentRoom.id
    exits = player.currentRoom.getExits()

    if room not in visited:
        visited[room] = {direction: '?' for direction in exits}
    ...
    unvisited_len = len(unvisited_rooms)

    if unvisited_len > 0:
        next_room_index = random.randint(0, unvisited_len - 1)
        next_dir = unvisited_rooms[next_room_index]

        player.travel(next_dir)
        traversalPath.append(next_dir)

        next_room = player.currentRoom.id
        visited[room][next_dir] = next_room

        if next_room not in visited:
            next_exits = player.currentRoom.getExits()
            visited[next_room] = {direction: '?' for direction in next_exits}

        opposite_dir = opposite_direction(next_dir)
        visited[next_room][opposite_dir] = room

    else:
        traversalPath += not_visited(visited, room, player)
